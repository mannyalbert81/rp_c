...
class ProcesarAnularComprobantesController extends ControladorBase{
	...
	public function index(){
		...
		$this->view_Contable("ProcesarAnularComprobantes",array());			
	
	}	
	
	
	/** dc 2021/02/12 **/
	public function dtMostrarComprobantes()
	{
	    if( !isset( $_SESSION ) ){
	        session_start();
	    }
	    ...
	    try {
	        ...
	        $columnas1 = " aa.id_ccomprobantes, aa.valor_letras, aa.valor_ccomprobantes, aa.numero_ccomprobantes, TO_CHAR(aa.fecha_ccomprobantes,'YYYY-MM-DD') fecha, 
                aa.referencia_doc_ccomprobantes, aa.observaciones_ccomprobantes, bb.nombre_entidades, cc.usuario_usuarios, dd.id_tipo_comprobantes, 
                dd.nombre_tipo_comprobantes, ee.nombre_forma_pago, ff.in_mayor, aa.aprobado_ccomprobantes,  aa.concepto_ccomprobantes";
	        $tablas1   = " public.ccomprobantes aa
    	        INNER JOIN public.entidades bb
    	        ON bb.id_entidades = aa.id_entidades
    	        INNER JOIN public.usuarios cc
    	        ON cc.id_usuarios = aa.id_usuarios
    	        INNER JOIN public.tipo_comprobantes dd
    	        ON dd.id_tipo_comprobantes = aa.id_tipo_comprobantes
    	        INNER JOIN public.forma_pago ee
    	        ON ee.id_forma_pago = aa.id_forma_pago
    	        LEFT JOIN (
                    SELECT id_ccomprobantes, 1 AS in_mayor
                 	FROM con_mayor 
                 	GROUP BY id_ccomprobantes
                ) ff
    	        ON ff.id_ccomprobantes = aa.id_ccomprobantes";
	        $where1    = " 1 = 1  AND  aa.transaccion_ccomprobantes not in  ('DESCUENTOS', 'credito', 'renovacion')";
	        ...
	        {
	            $where1 .= " AND ( ";
	            $where1 .= " aa.numero_ccomprobantes ILIKE '%$searchDataTable%' ";
	            $where1 .= " OR TO_CHAR(aa.fecha_ccomprobantes,'9999') ILIKE '%$searchDataTable%' ";
	            $where1 .= " ) ";
	        ...
	        $datos_provedor  = $_POST['proveedor'];
	       
	        if($id_entidades!=0){ $where1 .= " AND bb.id_entidades='$id_entidades'";}
	        
	        if($id_tipo_comprobantes!=0){ $where1 .= " AND dd.id_tipo_comprobantes='$id_tipo_comprobantes'";}
	        
	        if($numero_ccomprobantes!=""){ $where1 .= " AND aa.numero_ccomprobantes ILIKE '%$numero_ccomprobantes%'";}
	        
	        if($fecha_desde!="" && $fecha_hasta!=""){ $where1 .= " AND  date(aa.fecha_ccomprobantes) BETWEEN '$fecha_desde' AND '$fecha_hasta'";}
	        
	        if( strlen($datos_provedor ) > 0 ){
	            $where1 .= " AND aa.id_proveedores in ( ".
	   	            "     SELECT id_proveedores FROM proveedores ".
	   	            "WHERE identificacion_proveedores = '$datos_provedor' OR nombre_proveedores = '$datos_provedor' )";
	        }
	        ...
	        foreach ( $resultSet as $res){
	            ...
	            if( $res->aprobado_ccomprobantes == 'f' ){
	                $estadoComprobantes = "ANULADO";
	            }else if( $res->aprobado_ccomprobantes == 't' && empty($res->in_mayor) ){
	                $estadoComprobantes = "PENDIENTE";
	                $opciones = '<div class="pull-right ">
                            <span > 
                            <a onclick="proceso_contabilizar(this)" id="" data-id_comprobantes="'.$res->id_ccomprobantes.'" href="#" class=" no-padding btn btn-sm btn-default" data-toggle="tooltip" data-placement="right" title="Contabilizar"> <i class="fa  fa-external-link fa-2x fa-fw" aria-hidden="true" ></i>
	                           </a>
                            </span>
                            <span >
                                <a onclick="proceso_anular(this)" id="" data-id_comprobantes="'.$res->id_ccomprobantes.'" href="#" class=" no-padding btn btn-sm btn-default" data-toggle="tooltip" data-placement="right" title="Anular"> <i class="fa  fa-ban fa-2x fa-fw" aria-hidden="true" ></i>
	                           </a>
                            </span>                           
                            </div>';
	            }else if( $res->aprobado_ccomprobantes == 't' && !empty($res->in_mayor) ){
	                $estadoComprobantes = "CONTABILIZADO";
	                $opciones = '<div class="pull-right ">
                            <span >
                                <a onclick="proceso_anular(this)" id="" data-id_comprobantes="'.$res->id_ccomprobantes.'" href="#" class=" no-padding btn btn-sm btn-default" data-toggle="tooltip" data-placement="right" title="Anular"> <i class="fa  fa-ban fa-2x fa-fw" aria-hidden="true" ></i>
	                           </a>
	                ...
	                $estadoComprobantes = "N/D";
	            }
	            
	           
	            
	            $dataFila['numfila'] = $columnIndex;
	            $dataFila['nombre_entidad']  = $res->nombre_entidades;
	            $dataFila['tipo'] =  $res->nombre_tipo_comprobantes;;
	            $dataFila['concepto'] = $res->concepto_ccomprobantes;
	            $dataFila['numero']  = $res->numero_ccomprobantes;
	            $dataFila['valor']  = $res->valor_ccomprobantes;;
	            $dataFila['fecha']  = $res->fecha;
	            $dataFila['estado']= $estadoComprobantes;	            
	            $dataFila['opciones'] = $opciones;
	            //$dataFila['extra']  = $res->aprobado_ccomprobantes;
	...
	/** dc 2021/02/17 **/
	public function obtenerdetallescomprobantes(){
	    
	    $Comprobante   = new MayorModel();
	    
	    $respuesta = array();
	    
	    try {
	        
	        $id_comprobantes   = isset($_POST['id_comprobantes']) ? $_POST['id_comprobantes'] : 0;
	        
	        if($id_comprobantes == 0 ){ throw new Exception('Comprobante no recibido');}
	        
	        $col1  = " numero_ccomprobantes, fecha_ccomprobantes, valor_ccomprobantes, to_char(fecha_ccomprobantes, 'YYYYMM') fecha_validar";
	        $tab1  = " public.ccomprobantes";
	        $whe1  = " id_ccomprobantes = $id_comprobantes ";
	        $id1   = " numero_ccomprobantes";
	        $rscomprobantes    = $Comprobante->getCondiciones( $col1, $tab1, $whe1, $id1);
	        
	        if( empty($rscomprobantes) ){ throw new Exception('Comprobante no encontrado en la BD'); }
	        
	        $validaperiodo = $rscomprobantes[0]->fecha_validar;
	        
	        $col2  = "  1 existe, bb.nombre_estado, bb.id_estado";
	        $tab2  = " con_periodo aa
        	    INNER JOIN public.estado bb
        	       ON aa.id_estado = bb.id_estado";
	        $whe2  = " bb.tabla_estado = 'con_periodo'
                AND anio_periodo || lpad(mes_periodo::text,2,'0') = '$validaperiodo'";
	        $id2   = " aa.id_periodo";
	        $rsperiodo    = $Comprobante->getCondiciones( $col2, $tab2, $whe2, $id2);
	        
	        if( empty($rsperiodo) ){ throw new Exception('Periodo no encontrado'); }	        
	       
	        $nombre_estado = $rsperiodo[0]->nombre_estado;
	        
	        if( $nombre_estado == 'CERRADO'){ throw new Exception('Periodo cerrado'); }
	        
	        $col3  = " aa.numero_dcomprobantes, aa.descripcion_dcomprobantes, bb.codigo_plan_cuentas, bb.nombre_plan_cuentas, aa.debe_dcomprobantes, aa.haber_dcomprobantes";
	        $tab3  = " public.dcomprobantes aa INNER JOIN plan_cuentas bb ON aa.id_plan_cuentas = bb.id_plan_cuentas";
	        $whe3  = " id_ccomprobantes = $id_comprobantes ";
	        $id3   = " aa.id_dcomprobantes";
	        $rsdetallecomprobantes    = $Comprobante->getCondiciones( $col3, $tab3, $whe3, $id3);
	        
	        //if( empty($rsdetallecomprobantes) ){ throw new Exception('Detalle com'); }
	        
	        $respuesta['estatus'] = 1;
	        $respuesta['mensaje'] = "ok";
	        $respuesta['cabecera'] = $rscomprobantes;
	        $respuesta['detalle'] = $rsdetallecomprobantes;
	...
	/** dc 2021/02/17 **/
	public function ProcesaComprobantes(){
	    ...
	    try {
	        ...
	        $parametrosMayoriza    = "$id_comprobantes,null";
	        $consultaMayoriza      = $Comprobante->getconsultaPG($funcionMayoriza, $parametrosMayoriza);
	        $Comprobante->llamarconsultaPG($consultaMayoriza);
	        
	        $error = "";
	        $error = pg_last_error();
	        if(!empty($error)){ throw new Exception('Comprobante no Procesado');}
	       	        
	        echo json_encode(array('estatus'=>1,'mensaje'=>"Comprobante procesado"));
	        
	    } catch (Exception $e) {
	        echo json_encode(array('estatus'=>0,'mensaje'=>"Comprobante no procesado"));
	    }
	...
	/** dc 2021/02/17 **/
	public function AnulaComprobantes(){
	    ...
	    try {
	        ...
	        $funcionMayoriza       = "con_anula_comprobantes_mayoriza";
	        $parametrosMayoriza    = "$id_comprobantes,null";
	        $consultaMayoriza      = $Comprobante->getconsultaPG($funcionMayoriza, $parametrosMayoriza);
	        $Comprobante->llamarconsultaPG($consultaMayoriza);
	        ...
	        echo json_encode(array('estatus'=>1,'mensaje'=>"Comprobante anulado"));
	        
	    } catch (Exception $e) {
	        echo json_encode(array('estatus'=>0,'mensaje'=>"Comprobante no anulado"));
	    }
...
WHERE id_cuentas_pagar_impuestos = $id_cuentas_pagar_impuestos";
	...
	//dc 2020/07/23 funcion para renvio de XML RETENCION
	public function EnviarXmlRetencion_ClaveAcceso(){
