...
function InfoSolicitud(cedula,id_solicitud)
{
	...
	.done(function(x) {
		...
		var y=$.parseJSON(x);
		//console.log(y);
		$('#info_solicitud').html(y[0]);
		var_id_solicitud=y[1];
	    //$("#link_solicitud").data("id_solicitud",id_solicitud);
	   // console.log("valor de id -->"+id_solicitud);
		//Aportinfo_solicitudesParticipe(1),
...
function GuardaDesafiliación(){
	
	var id_TipoPrestaciones = $("#id_tipo_prestaciones");
	var fecha_prestaciones = $("#fecha_prestaciones");
	var observacion_prestaciones = $("#observacion_prestaciones");
	console.log(id_TipoPrestaciones.val() );
	console.log(fecha_prestaciones.val() );
	...
	$.ajax({
		...
		beforeSend:function(){$("#divLoaderPage").addClass("loader")},
		url:"index.php?controller=BuscarParticipesCesantes&action=GuardaDesafiliacion",
		type:"POST",
		data:{id_tipo_prestaciones:id_TipoPrestaciones.val(), 
			action:'ajax',
			id_participe:id_participe ,
			id_solicitud:var_id_solicitud,
			observacion_prestaciones:observacion_prestaciones.val(), 
			fecha_prestaciones:fecha_prestaciones.val()}
	}).done(function(datos){		
